                  ...
                  <div data-menu="true" style={{ position: 'absolute', bottom: '110%', left: 0, right: 0, background: '#161616', border: '0.5px solid #1e1e1e', borderRadius: '8px', overflow: 'hidden', zIndex: 100 }}>
                    {(['720p', '1080p'] as const).map(q => (
                      <div
                        ...
                        style={{ padding: '8px 12px', fontSize: '12px', color: '#555', background: 'transparent', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}
                      >
                        {q}
          ...
          {videos.map(v => (
            <div key={v.id} style={{ width: '100%', background: '#111', border: '0.5px solid #1e1e1e', borderRadius: '14px', overflow: 'hidden', flexShrink: 0, display: 'flex' }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ position: 'relative' }} onMouseEnter={e => { const el = e.currentTarget.querySelector('.video-actions') as HTMLElement; if (el) el.style.opacity = '1'; }} onMouseLeave={e => { const el = e.currentTarget.querySelector('.video-actions') as HTMLElement; if (el) el.style.opacity = '0'; }}>
                  <video src={v.videoUrl} controls loop style={{ width: '100%', display: 'block', maxHeight: '80vh', objectFit: 'contain', background: '#000' }} />
                  
                  {/* Liked persistence */}
                  {likedVideos.has(v.id) && (
                    <div style={{ position: 'absolute', top: '10px', right: '10px', zIndex: 20, pointerEvents: 'none' }}>
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="#532fcf" stroke="#532fcf" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  ...
                  {/* Hover actions */}
                  <div className="video-actions" style={{ position: 'absolute', top: '10px', right: '10px', display: 'flex', gap: '6px', opacity: 0, transition: 'opacity 0.2s', zIndex: 30 }}>
